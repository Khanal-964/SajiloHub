...
pool.getConnection()
  ...
  .catch((err) => {
    ...
    if (err.code === 'ER_ACCESS_DENIED_ERROR') {
      console.error('   → Check that DB_PASSWORD in server/.env matches your MySQL password.');
    } else if (err.code === 'ECONNREFUSED') {
      console.error('   → Is MySQL running? Check DB_HOST and DB_PORT in server/.env.');
    } else if (err.code === 'ER_BAD_DB_ERROR') {
      console.error('   → Database not found. Run the schema: mysql -u root -p < db/schema.sql');
    }
  });
